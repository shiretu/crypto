...
const writeTrade = (buf, offset, tsUs, index, price, baseQty, quoteQty, isBuyerMaker) => {
    Trade.writeRecord(buf, offset, tsUs, index, price, baseQty, quoteQty, isBuyerMaker)
}

describe('Trade', () => {
    it('should have RECORD_SIZE of 40', () => {
        expect(Trade.RECORD_SIZE).to.equal(40)
    })

    it('should read tsUs from buffer', () => {
        const buf = Buffer.alloc(Trade.RECORD_SIZE)
        writeTrade(buf, 0, 1704067200123456, 42, 42000, 0.5, 21000, false)
        const trade = new Trade(buf)
        expect(trade.tsUs).to.equal(1704067200123456)
    })

    it('should read index from buffer', () => {
        const buf = Buffer.alloc(Trade.RECORD_SIZE)
        writeTrade(buf, 0, 1000000, 12345678, 42000, 0.5, 21000, false)
        const trade = new Trade(buf)
        expect(trade.index).to.equal(12345678)
    })

    it('should read price from buffer', () => {
        const buf = Buffer.alloc(Trade.RECORD_SIZE)
        writeTrade(buf, 0, 1000000, 1, 42123.45, 0.1, 4212, false)
        const trade = new Trade(buf)
    ...
    it('should read baseQty from buffer', () => {
        const buf = Buffer.alloc(Trade.RECORD_SIZE)
        writeTrade(buf, 0, 1000000, 1, 42000, 1.2345, 51869, false)
        const trade = new Trade(buf)
    ...
    it('should read quoteQty from buffer', () => {
        const buf = Buffer.alloc(Trade.RECORD_SIZE)
        writeTrade(buf, 0, 1000000, 1, 42000, 0.5, 21000.75, false)
        const trade = new Trade(buf)
    ...
    it('should read isBuyerMaker false', () => {
        const buf = Buffer.alloc(Trade.RECORD_SIZE)
        writeTrade(buf, 0, 1000000, 1, 42000, 0.5, 21000, false)
        const trade = new Trade(buf)
    ...
    it('should read isBuyerMaker true', () => {
        const buf = Buffer.alloc(Trade.RECORD_SIZE)
        writeTrade(buf, 0, 1000000, 1, 42000, 0.5, 21000, true)
        const trade = new Trade(buf)
    ...
    it('should preserve tsUs when isBuyerMaker is true', () => {
        ...
        writeTrade(buf, 0, ts, 1, 42000, 0.5, 21000, true)
        const trade = new Trade(buf)
    ...
    it('should be a view on a subarray of a larger buffer', () => {
        ...
        writeTrade(big, 0, 1000000, 1, 100, 0.1, 10, false)
        writeTrade(big, Trade.RECORD_SIZE, 2000000, 2, 200, 0.2, 40, true)
        writeTrade(big, Trade.RECORD_SIZE * 2, 3000000, 3, 300, 0.3, 90, false)
