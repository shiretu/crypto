...
const createTable = async (db, exchangeName, symbolName) => {
    await db.query({ query: 'CREATE DATABASE IF NOT EXISTS market' })
    await db.query({ query: `CREATE TABLE IF NOT EXISTS ${tableName(exchangeName, symbolName)} (${getColumns().map(c => (`${c[0]} ${c[1]}`)).join(',')}) ENGINE = MergeTree ORDER BY (ts, id)` })
}

const getLatestAvailableDataTimestampUs = async (db, exchangeName, symbolName) => {
    const qr = await (await db.query({
        query: `SELECT coalesce(max(ts), 0) as maxTs FROM ${tableName(exchangeName, symbolName)}`,
        format: 'JSONEachRow'
    })).json()
...
const ensureData = async (dbConfig, symbolName, historySizeInDays) => {
    ...
    try {
        ...
        for (let dayMs = historyStartMs + dayDurationMs; dayMs <= yesterdayMs; dayMs += dayDurationMs) {
            ...
            await db.insert({
                table: tableName(exchangeName, symbolName),
