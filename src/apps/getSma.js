...
const usage = () => {
    console.error('Usage: getSma <exchange> <symbol> <duration> <period> [YYYY-MM-DD] [YYYY-MM-DD]')
    ...
    console.error('  period    : SMA period (e.g. 7)')
...
const main = async () => {
    ...
    console.log(`SMA(${period}) on ${durationArg} candles for ${symbol} from ${exchange.id}`)
    ...
    for await (const candle of store.readAsync(start, end)) {
        ...
        if (value !== null) {
            console.log(`${time}  close=${candle.close.price.toFixed(2)}  SMA=${value.toFixed(2)}`)
        }
    }
}
